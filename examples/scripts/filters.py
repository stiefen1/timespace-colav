...
ts = MovingShip.from_csog(
    ...
    du=0.1,                 # Speed uncertainty
    dchi=5                  # Course uncertainty
)
...
for i, projected_ship in enumerate(planner.projector.get(p0, info['pf'], [ts_with_sd])):
    ax.fill(*projected_ship.exterior.xy, c='orange', alpha=0.8, label=f"footprints" if i==0 else None)
...
if planner.colregs:
    ax.text(-46, -35, "target ship\ntimespace footprint", ha='center', fontsize=8.7)
else:    
    ax.text(-34, -24, "target ship\ntimespace footprint", ha='center', fontsize=8.7)
