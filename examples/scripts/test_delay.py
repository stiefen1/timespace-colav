...
# Projected footprint
for i, projected_ship in enumerate(planner.projector.get(p0, info['pf'], [ts1_with_sd, ts2_with_sd], delay=DELAY, delay_type='symmetric')):
    ax.fill(*projected_ship.exterior.xy, c='grey', alpha=0.7, label=f"footprints" if i==0 else None)
for i, projected_ship in enumerate(planner.projector.get(p0, info['pf'], [ts1_with_sd, ts2_with_sd], delay=DELAY, delay_type='flat')):
    ax.plot(*projected_ship.exterior.xy, c='blue', alpha=0.7, label=f"footprints" if i==0 else None)
for i, projected_ship in enumerate(planner.projector.get(p0, info['pf'], [ts1_with_sd, ts2_with_sd], delay=-DELAY, delay_type='flat')):
    ax.plot(*projected_ship.exterior.xy, c='red', alpha=0.7, label=f"footprints" if i==0 else None)
